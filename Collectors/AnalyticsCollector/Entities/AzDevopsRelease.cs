...
namespace AnalyticsCollector
{
    public class AzDevopsRelease : AzureDataExplorerService
    {
        ...
        public AzDevopsRelease(ReleaseRestAPIProvider releaseRestApiProvider, string serviceNameAndRegion, string aadTenantIdOrTenantName, string organizationName, string projectId)
        ...
        private void WriteData(StreamWriter writer, string waterMark, out int continuationTokenOutput, out DateTime minCreatedDateTime)
        {
            ...
            do
            {
                var releases = this._releaseRestApiProvider.GetReleases(minCreatedDateTime, continuationToken, out continuationTokenOutput, ReleaseExpands.Variables);
