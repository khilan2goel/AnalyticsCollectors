...
namespace AnalyticsCollector
{
    public class AzDevopsReleaseArtifact : AzureDataExplorerService
    {
        ...
        public AzDevopsReleaseArtifact(ReleaseRestAPIProvider releaseRestApiProvider, string serviceNameAndRegion, string aadTenantIdOrTenantName, string organizationName, string projectId)
        ...
        private void WriteData(StreamWriter writer, string waterMark, out int continuationTokenOutput, out DateTime minCreatedDateTime)
        {
            ...
            do
            {
                var releases = this._releaseRestApiProvider.GetReleases(minCreatedDateTime, continuationToken, out continuationTokenOutput, ReleaseExpands.Artifacts);
